...
public class TokenRepository implements TokenRepositoryType {
    ...
    private Web3j web3j;
    private CustomEosioJavaRpcProviderImpl customEosioJavaRpcProvider = null;
    ...
    public TokenRepository(OkHttpClient okHttpClient,
                           ...
                           CustomEosioJavaRpcProviderImpl customEosioJavaRpcProvider) {
        ...
        this.customEosioJavaRpcProvider = customEosioJavaRpcProvider;

        buildWeb3jClient(ethereumNetworkRepository.getDefaultNetwork());
    }

    private void buildWeb3jClient(NetworkInfo networkInfo) {
        LogUtils.dTag(LOG_TAG, "rpcServerUrl = " + networkInfo.rpcServerUrl);
    ...
    public Observable<Token[]> fetch(String walletAddress, Constants.WalletType walletType) {
        return Observable.create(e -> {
            ...
            Token[] tokens = tokenLocalSource.fetch(defaultNetwork, walletAddress, walletType)
                    .map(items -> {
            ...
            tokens = tokenLocalSource.fetch(defaultNetwork, walletAddress, walletType)
                    .map(items -> {
                        int len = items.length;
                        Token[] result = new Token[len];
                        for (int i = 0; i < len; i++) {
    ...
    @Override
    public Completable addToken(String walletAddress, String address, String symbol, int decimals, Constants.WalletType walletType) {
        ...
        return tokenLocalSource.put(ethereumNetworkRepository.getDefaultNetwork(),
                walletAddress, new TokenInfo(address, "", symbol, decimals, walletType));
    }
